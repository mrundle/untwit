...
$sql = "INSERT INTO user (UID, Name, Finished) VALUES (" . $UID . ", '" . $screenName . "', 0)";
...
foreach($friends as $friend){
	...
	/*while(True) {
		$friendName = $connection->get('friends/ids',array('user_id' => $friend));
		// check for rate limiting
		if ($connection->http_code === 429) {
			sleep(901);
		} else {
	...
	//$friendName = get_object_vars($friendName); // convert to array
	//$friendName = $friendName['screen_name'];
	
	//get screen name from the first tweet
	$friendName = '';
	foreach($tweets as $twt){
		if (isset($twt['user']['screen_name'])){
			echo "\nHOLYSHITITWORKS\n";
			$friendName = $twt['user']['screen_name'];
			// We just want the first tweet
			break;
		}
	}

	$sql = "INSERT INTO follow (UID, FriendID, FriendName) VALUES (" . $UID . ", " . $friend . ", '" . $friendName . "')";
	if ($conn->query($sql) === TRUE) {
	...
	foreach($tweets as $twt){
		...
		if ($result->num_rows > 0) {
			...
			if ($twtArray[0] == "RT"){ //first word in tweet is RT
               
				array_shift($twtArray);//remove RT
				$mentionID = array_shift($twtArray);//get second word in tweet
				$mentionID = substr($mentionID, 1);//get rid of @
				$mentionID = substr($mentionID, 0, -1);//getting rid of
...
// DONE!
$sql = "UPDATE user SET Finished = 1 WHERE UID = " . $UID;
if ($conn->query($sql) === TRUE) {
	echo "SUCCESS!!!";
} else {
    echo "Error: " . $sql . "<br>" . $conn->error . "<br>";
